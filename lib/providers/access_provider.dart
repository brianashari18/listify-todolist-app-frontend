...
class AccessNotifier extends StateNotifier<Map<String, dynamic>> {
  ...
  Future<void> editAccess(User user, Task task, Access access, int index) async {
    final result = await _workspaceService.addPeopleAccess(user, task, access);
    if (result['success'] == 'true') {
  ...
  Future<void> deleteAccess(User user, Task task, Access access, int index) async {
    final result = await _workspaceService.deletePeopleAccess(user, task, access);
    if (result['success'] == 'true') {
...
final accessProvider = StateNotifierProvider<AccessNotifier, Map<String, dynamic>>((ref) {
